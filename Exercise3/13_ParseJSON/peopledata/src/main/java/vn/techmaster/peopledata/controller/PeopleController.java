...
public class PeopleController {
  ...
  public Map<String,List<Person>> listDevInHanoiSaigonShanghai() {
    return personRepo.listDevInHanoiSaigonShanghai();
  }
  ...
  public ResponseEntity<List<Map.Entry<String,Double>>> youngestdevs() {
    return ResponseEntity.ok().body(personRepo.topCityDevHasYoungAvgAgeDev());
  }
  //Liệt kê tỷ lệ nam/nữ ở từng thành phố
  @GetMapping("malefemaleratioperCity")
  public  ResponseEntity<Map<String, Double>> malefemaleratioperCity() {
    return ResponseEntity.ok().body(personRepo.malefemaleratioperCity());
  }
  ...
  public ResponseEntity<Double> avgsalarypeopleabove30() {
    return ResponseEntity.ok().body(personRepo.calAvgSalaryOfPeopleAbove30());
  }
}