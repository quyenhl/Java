...
public class PersonRepository {
  ...
  public Double maleFemaleRatioCity(List<Person> people) {
    Long maleCount = people
            .stream()
            .filter(p -> p.getGender().equals("Male"))
            .collect(Collectors.counting());
  ...
  public Map<String, List<Person>> listDevInHanoiSaigonShanghai() {
    Map<String, List<Person>> groupDevelopersByCity = people
            .stream()
            .filter(p -> p.getJob().equals("developer"))
  ...
  public List<Map.Entry<String, Double>> topCityDevHasYoungAvgAgeDev() {

    Map<String, List<Person>> groupDevelopersByCity1 = people
            .stream()
            .filter(p -> p.getJob().equals("developer"))
            .collect(Collectors.groupingBy(Person::getCity));
    Map<String, Double> cityAverageAge = groupDevelopersByCity1.entrySet()
            .stream()
            .collect(Collectors.toMap(Map.Entry::getKey,
                    entry -> entry.getValue()
                            .stream()
  ...
  public Map<String, Double> malefemaleratioperCity() {
    return people
            .stream()
            .collect(Collectors.groupingBy(Person::getCity))
            .entrySet()
            ...
            .collect(Collectors.toMap(
                    entry -> entry.getKey(),
                    entry -> maleFemaleRatioCity(entry.getValue())));

  }

  //3Tính mức lương trung bình của tất cả những người trên 30 tuổi
  public Double calAvgSalaryOfPeopleAbove30(){
    return people
            .stream()
